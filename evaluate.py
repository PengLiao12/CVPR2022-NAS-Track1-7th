...
def run(
        ...
        phase=None,
        resume=None,
        pretrained='checkpoints/039193.pdparams',
        image_dir='/home/yumufeng/dataset/',
...
def main(cfg):
    ...
    if isinstance(archs, dict):
        # submit version
        arch_now = [archs[x]['arch'] for x in archs]
    else:
        arch_now = archs

    result = model.evaluate(arch_now, val_set, batch_size=cfg.batch_size, num_workers=8)

    print(result)
    # json.dump(result, open('45000_test.txt', 'w'))

    if isinstance(archs, dict):
        for key in archs:
            archs[key]['acc'] = result[archs[key]['arch'] ]

        json.dump(archs, open('45000_test.json', 'w'))
