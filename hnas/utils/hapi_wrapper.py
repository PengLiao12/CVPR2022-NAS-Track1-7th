...
import  time
from sampling import archs_sampling
...
class MyDynamicGraphAdapter(DynamicGraphAdapter):
    ...
    def train_batch(self, inputs, labels=None, **kwargs):
        ...
        plan_archs= kwargs.get('plan_archs', None)
        for i in range(self.dyna_bs):
            #print(self._nranks,nBatch,step,self.dyna_bs,plan_archs[step])
            ...
            self.model.network.active_subnet_arch(plan_archs[step],MyRandomResizedCrop.current_size)
            # print("MyRandomResizedCrop.current_size",MyRandomResizedCrop.current_size)
            # print("self.model.network.gen_subnet_code)",self.model.network.gen_subnet_code)
            if self._nranks > 1:
...
class Trainer(Model):
    ...
    def _run_one_epoch(self, data_loader, callbacks, mode, logs={}, **kwargs):    
        ...
        # print(ParallelEnv().local_rank, time_start)
        if mode == 'train':
        ...
        for step, data in enumerate(data_loader):
            ...
            # print("batch_size",batch_size, ParallelEnv().local_rank, time.time() - time_start)
            ...
            if mode != 'predict':
                if mode == 'train':
                    ...
                    plan_archs= archs_sampling(len(data_loader))
                    outs = getattr(self, mode + '_batch')(data[:len(self._inputs)],
                                                          data[len(self._inputs):], 
                                                          epoch=kwargs.get('epoch', None),
                                                          nBatch=len(data_loader),
                                                          step=step,
                                                           plan_archs=plan_archs)
                else:
    ...
    def evaluate(
        ...
        if eval_data is not None and isinstance(eval_data, Dataset):
            ...
            eval_loader = DataLoader(
                ...
                shuffle = False,
                num_workers=num_workers,
                return_list=True, use_buffer_reader=True, use_shared_memory=True)
        else:
            print("NO DistributedBatchSampler!!!")
            eval_loader = eval_data
        ...
        for arch in tqdm(archs):
            ...
            self.network.active_subnet_arch(arch, 224)
            ...
            results[arch] = eval_result['acc_top1']
